...
function outputPath() {
  return process.env.ALZA_OUTPUT_PATH || '';
}

async function writeOutput(obj) {
  const out = outputPath();
  if (!out) throw new Error('ALZA_OUTPUT_PATH missing');
...
async function ensureCookieFile(inputPath) {
  ...
  if (stat.isDirectory()) {
    ...
    const defaultDirect = path.join(expanded, 'Default', 'Cookies');
    if (await fileExists(defaultDirect)) return defaultDirect;
    const defaultNetwork = path.join(expanded, 'Default', 'Network', 'Cookies');
    if (await fileExists(defaultNetwork)) return defaultNetwork;
...
async function ensureCookiesDirForFallback(cookieFile) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'alza-chrome-cookies-'));
